...
public class TestMinSpanningTree {
	public static void main(String[] args) throws Exception {
		// 对文件testG.txt进行读取
		BufferedReader file = new BufferedReader(new InputStreamReader(new FileInputStream("Graph/prim/testG4.txt")));
...
//		System.out.println(Arrays.deepToString(arr));
		...
		System.out.println(prim.getMinWeight() + ", 时间: " + (end - start) + "ms");
		
		start = System.currentTimeMillis();
		PrimOptimized<Double> primOptimized = new PrimOptimized<>(denseGraph);
		end = System.currentTimeMillis();
		System.out.println(primOptimized.getMinWeight() + ", 时间: " + (end - start) + "ms");
		
		
		start = System.currentTimeMillis();
		KrusKal<Double> krusKal = new KrusKal<Double>(denseGraph);
		end = System.currentTimeMillis();
		System.out.println(krusKal.getMinWeight() + ", 时间: " + (end - start) + "ms");
